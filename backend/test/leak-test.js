...
async function main() {
  ...
  /* ---- a URL cannot walk out of frontend/ ----------------------------
     The generic-CRUD guard falls back to serving <name>.html out of
     frontend/ when the name is not a collection, and it used to build that
     ...
     The probe is a file that genuinely exists outside frontend/, because
  ...
  const outside = [
    "backend/lib/codeagent/scaffold/index.html",
    "node_modules/tslib/tslib.html",
    "backend/node_modules/tslib/tslib.html"
  ].find((p) => fs.existsSync(path.join(root, p)));

  /* This used to log "probe skipped" and carry on. That is a security test
     failing open: the day a folder is renamed under it, every path in the
     list misses, the traversal assertion never runs, and the suite still
     prints green. A probe that cannot find its target has not passed — it
     has not run, and the two must not look alike. */
  if (!outside) {
    throw new Error(
      "traversal probe found none of its target files — the paths above are " +
      "stale, so this assertion did not run. Fix the list; do not skip."
    );
  } else {
    ...
    if (got.length) fail("a URL walked out of frontend/ and served a file: /" + got[0]);
    else pass("none of " + attempts.length + " traversal spellings escaped frontend/ (probe: " + outside + ")");
