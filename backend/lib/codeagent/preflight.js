/* =================================================================
   preflight.js — the checks a compiler cannot make, made before it runs
   -----------------------------------------------------------------
...
function resolvePath(spec, fromPath) {
...
function importResolves(spec, fromPath, known) {
 ...
 *
 * Returns { hard, soft } — build-error shaped, ready for the repair loop.
 */
function preflight(files) {
  const tree = Object.assign({}, scaffoldFiles.readScaffold(), files || {});
  ...
  for (const [path, contents] of Object.entries(tree)) {
    ...
    if (CODE_RE.test(path)) {
      ...
      let m;
      FROM_RE.lastIndex = 0;
      while ((m = FROM_RE.exec(contents)) !== null) specs.push([m[1], m.index]);
      SIDE_EFFECT_RE.lastIndex = 0;
      while ((m = SIDE_EFFECT_RE.exec(contents)) !== null) specs.push([m[1], m.index]);
...
module.exports = { preflight, ALLOWED_PACKAGES };
