...
function run() {
  ...
  if (!envIsIgnored()) {
    err(".env is not gitignored. It holds every secret on this host, and the " +
        "next `git add -A` would commit it. Add `infra/deploy/.env` to .gitignore.");
  }
 ...
 * in infra/deploy/, or in .git/info/exclude — and a text scan of one file would
...
function envIsIgnored() {
  ...
  /* Walk up instead of guessing a depth. This used to look exactly two
     places — ROOT and ROOT/.. — and match the literal `deploy/.env`, which
     silently stopped finding the repo root the day the stack moved under
     infra/. A rule can sit in any ancestor's .gitignore, so ask each one
     whether it names this .env, either bare or by the path it would use. */
  let dir = ROOT;
  for (;;) {
    const f = path.join(dir, ".gitignore");
    if (fs.existsSync(f)) {
      const rel = path.relative(dir, ENV_PATH).split(path.sep).join("/");
      const lines = fs.readFileSync(f, "utf8")
        .split(/\r?\n/)
        .map((l) => l.trim());
      if (lines.includes(".env") || lines.includes(rel) || lines.includes("/" + rel)) return true;
    }
...
function generate() {
  ...
  if (!env.JWT_SECRET) {
    ...
    console.log("    grep JWT_SECRET ../../backend/.env");
